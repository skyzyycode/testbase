module.exports = {
	  ...
	  async run(m, { sock, Func, text, config }) {
		  if (!Func.isUrl(text) || !text) throw `> Ketik atau reply link saluran WhatsApp`
		  for (let prop of Func.isUrl(text)) {
			   if (!/whatsapp.com\/channel/.test(prop)) continue
			  m.reply(config.messages.wait);
			  let id = prop.replace(new RegExp(/https:\/\/\whatsapp.com\/channel\//,
			  "gi"), '')
...
> *- Dibuat sejak :* ${new Date(metadata.creation_time *
1000).toLocaleString("ID")}`
			  m.reply(metadata.preview ? {
				  image: {
