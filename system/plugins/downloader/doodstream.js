module.exports = {
     ...
     if (m.text.includes("--upload")) {
        ...
        if (!/video/.test(q.msg.mimetype) || !q.isMedia) throw `> Reply/kirim video Dengan caption ${m.prefix + m.command} ${text}` 
       ...
       if (!Func.isUrl(text) || !/dood.(li|la|com|ws)/.test(text)) throw "> Masukan link Doodstream nya"
     ...
     if (limit.oversize) throw `Ukuran File terlalu besar *( ${size} )*, Uprgade ke premium agar dapat download ukuran video hingga *1GB !*`
