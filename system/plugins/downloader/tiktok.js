module.exports = {
      ...
      settings: {
       ...
       async run(m, { sock, Func, text, Scraper, config }) {
        if (!Func.isUrl(m.text) || !/tiktok.com/.test(m.text) || !m.text) throw `> Reply atau masukan link tiktok yang ingin di download`
     ...
     await Scraper.tiktok.download(m.text).then(async(a) => {
      ...
      if (limit.oversize) return m.reply(`> Maaf Video tidak Dapat diputar karena melebihi maksimal ukuran *( ${size} )*, Maksimal ukuran untuk pengguna Free adalah *50MB*, Upgrade ke premium agar dapat meningkatkan maksimal ukuran hingga *1GB* !`);
