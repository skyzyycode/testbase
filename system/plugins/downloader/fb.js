...
module.exports = {
     ...
     async run(m, { sock, Scraper, Text, Func, text }) {
       if (!/facebook.com|fb.watch/.test(text) || !text) throw "> Masukan link Facebook nya"
       let data = await Scraper.facebook(text)
       let random = data.media[0]
       let buffer = await fetch(random).then(async(a) => Buffer.from(await a.arrayBuffer()));
       let size = Func.formatSize(buffer.length);
       let limit = await Func.sizeLimit(size, db.list().settings.max_upload);
       if (limit.oversize) throw `Maaf saya tidak dapat mengunduh video Facebook tersebut karena ukuran video melebihi batas ukuran yang di tentukan *( ${size} )*, Upgrade status mu ke premium agar dapat batas maksimal hingga *1GB* !`
