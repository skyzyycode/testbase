       	...
       	let exif;
			...
			let [packname, author] = m.text.split(/[,|\-+&]/);
			 	exif = { packName: packname ? packname : '', packPublish: author ? author : '' };
						...
						exif = { packName: config.sticker.packname, packPublish: config.sticker.author };
					}
		      	 let sticker = await writeExif({ mimetype: quoted.msg.mimetype, data: media }, exif);
					...
					for (let id of m.mentions) {
						...
						let sticker = await writeExif(media, { packName: config.sticker.packname, packPublish: config.sticker.author });
						await m.reply({ sticker });
					}
