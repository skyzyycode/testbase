...
module.exports = async(messages, sock, store) => {
	...
	m.isOwner =  [sock.user.jid,...(config.owner.map(a => a + "@s.whatsapp.net"))].includes(m.sender)
	if (m.message) {
    ...
    let msg = {
      ...
      type: "append",
       ...
       return sock.ev.emit("messages.upsert", msg);
				...
				m.quoted.isOwner = [sock.user.jid, ...(config.owner.map(a => a + "@s.whatsapp.net"))].includes(m.sender)
				   //==========[ Function bot ]=============//
if (m.quoted.isMedia) {
